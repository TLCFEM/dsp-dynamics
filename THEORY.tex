...
\begin{gather}
\hat{F_v}\left(\omega\right)=\hat{p}\left(\omega\right)\cdot\dfrac{i\left(2\zeta\beta\right)}{\left(1-\beta^2\right)+i\left(2\zeta\beta\right)}.
\end{gather}
...
so that
\begin{gather}
\hat{F_v}\left(\omega\right)=\hat{p}\left(\omega\right)\cdot\dfrac{i\left(2a_0\omega\right)}{\left(\omega_n^2-\omega^2\right)+i\left(2a_0\omega\right)}.
\end{gather}
...
so that
\begin{gather}
\hat{F_v}\left(\omega\right)=\hat{p}\left(\omega\right)\cdot\dfrac{i\left(2a_1\omega\right)}{\left(1-\beta^2\right)+i\left(2a_1\omega\right)}.
\end{gather}

It can be noticed that all three expressions of $\hat{F_v}\left(\omega\right)$ possess a similar form
\begin{gather}\label{eq:general_damping_force}
\hat{F_v}\left(\omega\right)=\hat{p}\left(\omega\right)\cdot\dfrac{iA}{B+iA},
\end{gather}
and $B=0$ when $\omega=\omega_n$, where the maximum magnitude \num{1} is achieved. At this exact point, the magnitude of damping force component simply equals that of the external load component. The magnitude distribution of $\hat{k_v}\left(\omega\right)$ in frequency domain can be computed and illustrated in \figref{fig:constant_damping}, \figref{fig:k_proportional} and \figref{fig:m_proportional}.
\begin{figure}[htb!]
\centering
...
\end{gather}
where $\hat{p_e}\left(f\right)$ is the spectrum of zero--stuffed external load that possesses identical magnitude at imaging frequencies, see \figref{fig:extended}.
...
where
\begin{gather}
\hat{k_a}\left(\omega\right)=-m\omega^2\cdot\hat{h}\left(\omega\right)=\dfrac{-\beta^2}{\left(1-\beta^2\right)+i\left(2\zeta\beta\right)}.
\end{gather}
...
It is then possible to evaluate the possibility of such an overshoot over the whole frequency domain. One example is given in \figref{fig:map}.
\begin{figure}[htb!]
\centering
...
\end{subfigure}
\begin{subfigure}{.48\textwidth}
\includegraphics{PIC/ConstantMapBlackmanharris-2000}
\caption{Blackman--Harris window}
\end{subfigure}
\caption{normalised damping force magnitude with constant damping ratio}\label{fig:map}
\end{figure}
